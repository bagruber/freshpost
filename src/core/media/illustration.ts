import { recolorSvg, type ColorMap } from "../color/svgRecolor";
...
// Quelle fürs <img>: SVG ggf. umgefärbt (Abbildung kommt von der Marke),
// PNG unverändert — an einem Foto darf der harte Hue-Snap nicht arbeiten.
export function illuSrc(illu: Illu, recolor: boolean, map: ColorMap): string {
  if (illu.isSvg) return svgDataUrl(recolor ? recolorSvg(illu.svgText!, map) : illu.svgText!);
  return illu.pngUrl!;
}
